...
class ListaMuseosFavPage extends StatefulWidget {
  static final String route = "lista_museos";

  ListaMuseosFavPage({Key key}) : super(key: key);

  @override
  _ListaMuseosPageState createState() => _ListaMuseosPageState();
}

class _ListaMuseosPageState extends State<ListaMuseosFavPage> {
  ...
  void getData() {
    museosProvider.getMuseos().then((value) {
      setState(() {
        museos.clear();
        museos.addAll(value.where(
            (element) => favs?.contains(element.id.toString()) ?? false));
      });
      if (museos.length != favs.length) {
        getData();
      }
    });
  }
